...
- **Catchall vs specific ambiguity is a corpus artifact, not a retrieval failure.**
  ...
  patterns (`I`, `NONE`, `M+I`).
    ...
    with st.expander("How accuracy is measured (grouped MRR)"):
        ...
        st.markdown(
            "Because LOINC axes do not uniquely identify codes, particularly the specimen system "
            "axis (where manufacturers chose inconsistently between specific codes like `Nph` and "
            "catchall codes like `Respiratory System Specimen` for clinically identical tests), "
            "exact-match MRR would penalize correct predictions. **Grouped MRR** instead defines an "
            "equivalence set per query of all clinically valid codes (same component and method, "
            "compatible specimen system); a prediction counts as correct if it lands in that set. "
            "This separates genuine retrieval failures from device registry artifacts."
        )
